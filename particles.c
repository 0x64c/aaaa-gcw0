...
void visparticles(void)
{
    ...
    zlBeginQuads();
    if (playmode==ZPM_ZVERYLE) zlBindTexture(180);
    else zlBindTexture(3);
    for (i=0; i<128; i++)
        if (pr[i][0]>0)
        {
            if (thisframenice)
            ...
            else if (x>=mpheader[8]) block=1;
            else if (y>=mpheader[8]) block=1;
            else if (z>=mpheader[9]) block=1;
            else if (mp[x][y][z][0]>0) block=1;
