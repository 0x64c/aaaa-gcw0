...
void trycheat(void)
{
    ...
    if (button[0]==1 | button[1]==1 | button[3]==1)
    {
        ...
        else if (button[1]==1) cheatinput[7]=2;
        else if (button[3]==1) cheatinput[7]=4;
//digestion
...
void gamemenukey(void)
{
    ...
    if (mmpos<0) mmpos=0;
    else if (mmpos>128*7) mmpos=128*7;
    mmlastsel=mmsel;
    mmsel =(mmpos+64)/128;
...
void menuconfigcontrol(void)
{
    ...
    newaxisact=0;
    if (axis[1]>60) newaxisact=2;
    else if (axis[1]<-60) newaxisact=1;

    if (newaxisact!=axisact)
    {
        axisact=newaxisact;
        if (axisact==1) if (confsel>0) {
                zcplaysound(3);
                confsel--;
            }
        if (axisact==2) if (confsel<7) {
                zcplaysound(3);
                confsel++;
            }
    }
    ...
    if (configdata[13]) sprintf(conftxt[6],"FILM GRAIN <ON>");
    else sprintf(conftxt[6],"FILM GRAIN <OFF>");
...
void gameselector(void)
{
    ...
    if (selsd<0) selsd=0;
    else if (selsd>7) selsd=7;
    if (selsd!=oldselsd) {
        zcplaysound(3);
        oldselsd=selsd;
    }

    if (selectordata<-256) selectordata=-256;
    else if (selectordata>3000) selectordata=3000;
